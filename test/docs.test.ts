...
async function guardedTools(): Promise<string[]> {
  // With IMAP_READ_ONLY defaulting to true the write tools are not registered
  // at all, so the default harness would compare the page against six of eleven
  // tools and call the five documented guards a lie.
  const client = (await connect({ config: { readOnly: false } })).client;
